...
```{r}
ggplot(X) +
  aes(x = age, y = vocab, color = gender) +
  geom_smooth()
```
...
```{r}
ggplot(X) +
  aes(x = age, y = vocab, color = nativeBorn) +
  geom_smooth()
```
...
```{r}
# The graph suggests a relationship where the older we get the more our vocabulary increases. This makes sense because as we grow older, we expereince more vocabulary.
```
...
R's built in function differs from the the optimization call.

Interpret the value of b_1 from R's built-in function.

when x increases by 1, the log odds of having diabetes increases by 0.4

Interpret the value of b_0 from R's built-in function.

When the variables are 0 the log odds are -5.504 or 0.004
...
It did pretty bad because for the ones, it predicted yes for 0.5 and predicted no for 0.22, which is better but still relatively did not perform well.
...
Appoximately performed similarly
...
# Fit probit regression model on training
...
# Print Brier scores for respective models
...
Since their in sample was about the same, the probably performed similarly too.
