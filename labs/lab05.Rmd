...
orthogonal_projection = function(a, v){
  scalar_multiplier = sum(a * v) / sum(v * v)
  a_parallel = scalar_multiplier * v
  a_perpendicular = a - a_parallel
  return(list(a_parallel = a_parallel, a_perpendicular = a_perpendicular))
}
```
...
# expect_equal(X %*% solve(t(X) %*% X) %*% t(X) %*% y, diag(n))
```
...
Not expected. The yhat_naive is substantially larger than yhat
...
for (j in 2:ncol(X)) {
  ...
  for (k in 1:(j-1)) {
    v_k = V[, k, drop = FALSE]
    proj_result = orthogonal_projection(X[, j], v_k)
    V[, j] = V[, j] - proj_result$a_parallel
  }
}
...
Yes, they are similar but shouldnt be exactly the same.
...
expect_equal(y_hat_Q, y_hat_of_R)
...
y_hat_whole_space = Q %*% t(Q) %*% y
is_projection_same = all.equal(yhat_naive, y_hat_whole_space)
...
```{r}
b = solve(t(X_train) %*% X_train) %*% t(X_train) %*% y_train # formula to calculate b
...
for (j in 1:p) {
  ...
  X_with_junk_test_j = X_with_junk_test[ , 1:j, drop = FALSE]
  yhat_test = X_with_junk_test_j %*% b
  e_test = y_test - yhat_test
...
pacman::p_load(ggplot2)
ggplot(data.frame(
    s_e = ooss_e_by_p, #we are taking the sd over all n oos residuals
    p = 1 : n_train
  )) +
