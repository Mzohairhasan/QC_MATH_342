...
oosRMSEs = array(NA, M)
for (i in 1:M) {
  model = lm(as.formula(model_formulas[i]), data = diamonds_train)
  dfs[i] = df.residual(model)
  predictions = predict(model, newdata = diamonds_select)
  oosRMSEs[i] = rmse(diamonds_select$price, predictions)
}
results = data.frame(Model_Formula = model_formulas, DF = dfs, oosRMSE = oosRMSEs)
print(results)
```
...
ggplot(data.frame(df = dfs, oosRMSE = oosRMSEs)+aes(x=df, y=oosRMSE)+
         geom_line()+
         geom_point()+
         labs(x = "Model Complexity (Degrees of Freedom)", y = "Out-of-Sample RMSE") +
  ggtitle("Out-of-Sample RMSE by Model Complexity")
```

Select the best model by oosRMSE and find its oosRMSE on the test set.

```{r}
n_start = which.min (oosRMSEs)
oosRMSEs[m_star]
...
No because the RMSE on the select is less than the select on the test.

Create the final model object `g_final`.

```{r}
#g_final takes an xstar and prints out a yhat
...
repeat {
  ...
  oos_brier = sum(-(y_select - y_hat_select)^2)
  ...
  if (i == 7){
    break #why?? because we dont want it to go on
  }
}
```
...
```{r}
unique_values = lapply(storms, unique)
storms = storms %>%
  mutate(
    name = as.factor(name),
...
storms = storms %>%
  mutate(
    average_diameter = rowMeans(select(., c("tropicalstorm_force_diameter", "hurricane_force_diameter")), na.rm = TRUE)
  )
head(storms)
```
...
```{r}
strongest_storm_per_year = storms %>%
  group_by(year) %>%
  filter(wind == max(wind, na.rm = TRUE))
strongest_storm_per_year
```
...
```{r}
max_values_per_storm = storms %>%
  group_by(name) %>%
  summarize(
    max_category = ifelse(all(is.na(category)), NA, max(category, na.rm = TRUE)),
    max_wind_speed = ifelse(all(is.na(wind)), NA, max(wind, na.rm = TRUE)),
    min_pressure = ifelse(all(is.na(pressure)), NA, min(pressure, na.rm = TRUE)),
    max_tropicalstorm_diameter = ifelse(all(is.na(tropicalstorm_force_diameter)), NA, max(tropicalstorm_force_diameter, na.rm = TRUE)),
    max_hurricane_diameter = ifelse(all(is.na(hurricane_force_diameter)), NA, max(hurricane_force_diameter, na.rm = TRUE))
  )
max_values_per_storm
```
...
storms = storms %>%
  mutate(
    datetime = as.POSIXct(paste(as.character(year), as.character(month), as.character(day), as.character(hour)), format = "%Y-%m-%d %H:%M:%S")
  )
storm_duration = storms %>%
  ...
  summarize(
    ...
    duration_hours = round(as.numeric(difftime(end_time, start_time, units = "hours")) / 6) * 6
  )
storm_duration
```
...
library(dplyr)
storms = storms %>%
  arrange(category, year, month, day, hour)
storms = storms %>%
  group_by(category) %>%
...
storms = storms %>%
  mutate(
    day_of_week = factor(weekdays(timestamp), levels = c("Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday")),
...
```{r}
storms = storms %>%
  mutate(start_day = format(timestamp, "%A, %B %d, %Y"))
storms
...
category_averages = storms %>%
  ...
  summarize(
    ...
    avg_tropicalstorm_force_diameter = mean(tropicalstorm_force_diameter, na.rm = TRUE),
    avg_hurricane_force_diameter = mean(hurricane_force_diameter, na.rm = TRUE)
  )
category_averages
...
storms = storms %>%
  mutate(distance_to_miami = calculate_distance(lat, long, MIAMI_LAT_LONG_COORDS[1], MIAMI_LAT_LONG_COORDS[2]))
storms
```
...
```{r}
calculate_distance_moved = function(lat1, lon1, lat2, lon2) {
  dist = distHaversine(c(lon1, lat1), c(lon2, lat2))
  return(dist)
}
storms = storms %>%
  ...
  group_by(name) %>%
  mutate(
    distance_moved = calculate_distance_moved(lat, long, lag(lat), lag(long))
  )
storms
...
storm_summary = storms %>%
  ...
  arrange(timestamp) %>%
  mutate(
    distance_moved = calculate_distance(lat, long, lag(lat), lag(long)),
    displacement = calculate_distance(lat[1], long[1], lat[n()], long[n()])
  ) %>%
  summarize(
    total_distance_moved = sum(distance_moved, na.rm = TRUE),
    total_displacement = max(displacement, na.rm = TRUE)
  )
storm_summary
```
...
```{r}
lm_model = lm(max_wind_speed ~ ., data = design_matrix)
summary(lm_model)
par(mfrow = c(2, 2))
plot(lm_model)
...
I'd say the success level is good. We successfully created a linear regression model to predict maximum wind speed based on the provided features.
...
```{r}
filtered_storms = storms %>%
  filter(!is.na(tropicalstorm_force_diameter) & !is.na(hurricane_force_diameter) &
         tropicalstorm_force_diameter != 0 & hurricane_force_diameter != 0)
head(filtered_storms)
...
```{r}
subset_storms = filtered_storms %>%
  group_by(name) %>%
  mutate(observation_period = row_number()) %>%
  ungroup() %>%
  select(name, observation_period, tropicalstorm_force_diameter, hurricane_force_diameter)
head(subset_storms)
...
```{r}
hu_diameters = subset_storms %>%
  select(name, observation_period, hurricane_force_diameter) %>%
  mutate(diameter_type = "hu") %>%
  rename(diameter = hurricane_force_diameter)
ts_diameters = subset_storms %>%
  select(name, observation_period, tropicalstorm_force_diameter) %>%
  mutate(diameter_type = "ts") %>%
  rename(diameter = tropicalstorm_force_diameter)
combined_diameters = bind_rows(hu_diameters, ts_diameters)
head(combined_diameters)
...
```{r}
set.seed(123)
random_storms = sample(unique(combined_diameters$name), 4)
filtered_data = combined_diameters %>%
  filter(name %in% random_storms)

# Plot
ggplot(filtered_data, aes(x = observation_period, y = diameter, color = diameter_type)) +
  geom_line() +
  facet_wrap(~ name, nrow = 2) +
  labs(x = "Observation Period", y = "Diameter", color = "Diameter Type") +
  theme_minimal()
```